...
export const fetchGames = cache(async (filters = {}) => {
  try {
    ...
    if (sName) {
      ...
      if (isNumeric) {
        ...
        return [{
          ...
          // Adicionar sprites básicos para o card
          sprites: {
        ...
        // Buscar por nome (parcial) - Aumentado para 200 resultados iniciais
    ...
    const pokemonDetails = await Promise.all(
      data.results.map(async (pokemonRef) => {
    ...
    // Aplicar filtros adicionais (mantido como antes)
    ...
    if (sWeakness) {
      ...
      filteredPokemon = filteredPokemon.filter(p => 
        p.types.some(type => (weaknessMap[type.toLowerCase()] || []).includes(sWeakness.toLowerCase()))
      );
    }
    ...
    if (sHeight) {
      switch (sHeight) {
        case 'small': filteredPokemon = filteredPokemon.filter(p => p.height < 1); break;
        case 'medium': filteredPokemon = filteredPokemon.filter(p => p.height >= 1 && p.height <= 2); break;
        case 'large': filteredPokemon = filteredPokemon.filter(p => p.height > 2); break;
      }
    }
    
    if (sWeight) {
      switch (sWeight) {
        case 'light': filteredPokemon = filteredPokemon.filter(p => p.weight < 10); break;
        case 'medium': filteredPokemon = filteredPokemon.filter(p => p.weight >= 10 && p.weight <= 50); break;
        case 'heavy': filteredPokemon = filteredPokemon.filter(p => p.weight > 50); break;
      }
    }
    
    if (sGeneration) {
      const generationMap = { 1: 'generation-i', 2: 'generation-ii', 3: 'generation-iii', 4: 'generation-iv', 5: 'generation-v', 6: 'generation-vi', 7: 'generation-vii', 8: 'generation-viii', 9: 'generation-ix' };
      const generationName = generationMap[sGeneration];
      if (generationName) {
        filteredPokemon = filteredPokemon.filter(p => p.generation === generationName);
      }
    }
    
    if (sRegion) {
      const regionMap = { 1: 'generation-i', 2: 'generation-ii', 3: 'generation-iii', 4: 'generation-iv', 5: 'generation-v', 6: 'generation-vi', 7: 'generation-vii', 8: 'generation-viii', 9: 'generation-ix' };
    ...
    if (sHabitat) {
      const habitatMap = { 1: 'cave', 2: 'forest', 3: 'mountain', 4: 'grassland', 5: 'waters-edge', 6: 'sea', 7: 'urban', 8: 'rare', 9: 'rough-terrain' };
      const habitatName = habitatMap[sHabitat];
      if (habitatName) {
        filteredPokemon = filteredPokemon.filter(p => p.habitat === habitatName);
      }
    }
...
// Função auxiliar para ordenar a lista de Pokémon (mantida como antes)
function sortPokemonList(pokemonList, ordering) {
  ...
  if (ordering) {
    switch (ordering) {
      ...
      case 'height': sortedList.sort((a, b) => a.height - b.height); break;
      case '-height': sortedList.sort((a, b) => b.height - a.height); break;
      case 'weight': sortedList.sort((a, b) => a.weight - b.weight); break;
      case '-weight': sortedList.sort((a, b) => b.weight - a.weight); break;
...
// Função para buscar detalhes de um jogo específico (Pokémon) - Mantida como antes
export const fetchGameDetails = cache(async (id) => {
  try {
    if (!id) throw new Error("ID do Pokémon não fornecido");
    const response = await fetch(`https://pokeapi.co/api/v2/pokemon/${id}`);
    if (!response.ok) throw new Error(`Pokémon com ID ${id} não encontrado`);
    const pokemon = await response.json();
    const speciesResponse = await fetch(pokemon.species.url);
    const speciesData = await speciesResponse.json();
    ...
    return {
      ...
      height: pokemon.height / 10,
      weight: pokemon.weight / 10,
      types: pokemon.types.map(t => ({ name: t.type.name, url: t.type.url })),
      abilities: pokemon.abilities.map(a => ({ name: a.ability.name, url: a.ability.url, is_hidden: a.is_hidden })),
      stats: pokemon.stats.map(s => ({ name: s.stat.name, value: s.base_stat })),
      moves: pokemon.moves.slice(0, 10).map(m => ({ name: m.move.name, url: m.move.url })),
      species: { name: speciesData.name, url: pokemon.species.url },
      generation: speciesData.generation.name,
      habitat: speciesData.habitat?.name || 'unknown',
