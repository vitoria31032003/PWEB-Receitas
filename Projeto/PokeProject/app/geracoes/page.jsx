...
export default function GeracoesPage() {
  ...
  const [filters, setFilters] = useState({ 
    ...
    sHeight: ".",
    sWeight: ".",
    sGeneration: ".", // Será definido quando uma geração for selecionada
    sOrdering: ".", 
    sPage: 1 
  });
  ...
  useEffect(() => {
    // Simular carregamento inicial da lista de gerações
  ...
  // Carregar Pokémon quando uma geração é selecionada ou filtros mudam
  useEffect(() => {
    if (!selectedGeneration) return; // Não busca se nenhuma geração está selecionada

    const fetchData = async () => {
      setLoading(true); // Indica carregamento de busca/filtro
      setIsLoadingMore(false); // Reseta loading de rolagem
      try {
        // Passa o ID da geração para o filtro sGeneration
        const currentFilters = { ...filters, sGeneration: selectedGeneration }; 
        const data = await fetchGames(currentFilters);
        
        if (currentFilters.sPage === 1) {
          setPokemonList(data);
          setTotalCount(data.length); // Atualiza a contagem inicial
        } else {
    ...
    fetchData();
  }, [selectedGeneration, filters]); // Re-executa quando a geração ou filtros mudam
  ...
  const resetFilters = () => {
    setFilters({ 
      ...
      sHeight: ".",
      sWeight: ".",
      sGeneration: selectedGeneration, // Mantém a geração selecionada
      sOrdering: ".", 
      sPage: 1 
    });
    setPokemonList([]);
  ...
  // Função para selecionar uma geração
  ...
  const backToGenerations = () => {
    ...
    setPokemonList([]);
    setFilters({ sName: ".", sType: ".", sWeakness: ".", sAbility: ".", sHeight: ".", sWeight: ".", sGeneration: ".", sOrdering: ".", sPage: 1 });
  };
  ...
  if (selectedGeneration) {
    ...
    const generationColor = generationInfo ? generationInfo.color : 'bg-gray-500';
    const textColor = generationInfo ? generationColor.replace('bg-', 'text-') : 'text-gray-800';
    ...
    return (
      <div className="min-h-screen bg-white py-9 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          {/* Barra de Filtros (similar à de Tipos, adaptada) */}
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-4">
              <div className="flex-grow relative">
                ...
                <input 
                  ...
                  type="text" 
                  name="sName" 
                  placeholder={`Buscar em ${generationInfo?.name}...`} 
                  value={filters.sName}
          ...
          {!hasMore && pokemonList.length > 0 && (
            <div className="text-center py-8">
              <p className="text-gray-600">Fim da lista de Pokémon para esta geração e filtros.</p>
            </div>
          )}
        </div>
      </div>
  ...
  // ----- Renderização da Lista de Gerações (Inicial) -----
  return (
    <div className="min-h-screen bg-white py-12 px-4">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-4xl font-bold mb-8 text-center">Gerações de Pokémon</h1>
        <p className="text-lg text-gray-700 text-center mb-12">
          Clique em uma geração para ver todos os Pokémon correspondentes e aplicar filtros adicionais.
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {pokemonGenerations.map((gen) => (
            ...
            >
              <div className={`${gen.color} h-full`}>
                <div className="p-6 text-white">
                  ...
                  <div className="flex justify-between items-center mt-4">
                    <span className="text-sm font-semibold">Explorar geração</span>
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
