...
// Mapeamento de tipos para classes CSS e cores de fundo
const typeStyles = {
...
export default function PokemonDetails({ params }) {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [params.id]); // Re-executa quando o ID na URL muda
  ...
  // Determinar a cor principal baseada no primeiro tipo
  const mainType = types.length > 0 ? types[0].name.toLowerCase() : 'normal';
  const mainBgColor = typeStyles[mainType]?.bg || 'bg-gray-500';

  // Determinar IDs de navegação
  const prevId = currentId > 1 ? currentId - 1 : null;
  // Ajustar limite máximo se necessário (ex: Gen 9 tem 1025)
    ...
    <div className="min-h-screen py-8 md:py-12 px-4 bg-gray-100">
      ...
      <div className={`max-w-5xl mx-auto bg-white rounded-xl overflow-hidden shadow-lg border-t-8 ${mainBgColor.replace('bg-', 'border-')}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 md:p-8">
          ...
          {/* Coluna Direita: Informações, Habilidades, Tipos, Stats */}
          <div>
            {/* Informações Básicas */}
        ...
        {/* Navegação Inferior (Repetida para conveniência) */}
        <div className="p-4 bg-gray-100 border-t border-gray-200 flex justify-between items-center">
           {prevId ? (
             ...
             <div className="w-24"></div> // Placeholder para manter o espaçamento
           )}
