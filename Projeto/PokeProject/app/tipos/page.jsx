...
export default function TiposPage() {
  ...
  const updateFilters = (event) => {
    ...
    // Para outros filtros, o useEffect já pega a mudança
  };

  const resetFilters = () => {
    setFilters({ // Mantém a página 1, mas limpa os outros filtros
      sName: '',
      sAbility: '',
      sHeight: '',
      sWeight: '',
    ...
    // O useEffect tratará da busca com filtros limpos
  };

  const applyFilters = () => {
    // Força a re-busca resetando a página (se já não estiver em 1)
    if (filters.sPage !== 1) {
        setFilters(prev => ({ ...prev, sPage: 1 }));
    } else {
        // Se já está na página 1, força a re-renderização do useEffect
        // clonando o objeto de filtros (embora geralmente não necessário)
  ...
  // Função para selecionar um tipo (passa o objeto completo)
  const handleSelectType = (type) => {
    ...
    setPokemonList([]); // Limpa a lista antiga
    ...
    return (
      <div className="min-h-screen bg-white py-9 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          {/* Descrição do Tipo */}
          <div className={`${typeColor} text-white p-4 rounded-lg mb-8 shadow`}>
            ...
            <div className="mb-3 sm:mb-0">
              {!loading && pokemonList.length > 0 && (
                <p className="text-gray-600 text-sm">
                  Exibindo {pokemonList.length} Pokémon do tipo {selectedType.name}
                </p>
              )}
              {!loading && pokemonList.length === 0 && (
            ...
            <>
              {pokemonList.length > 0 ? (
                ...
                !loading && (
                  <div className="text-center py-12 text-gray-500">
                    <p>Nenhum Pokémon encontrado com os filtros selecionados para o tipo {selectedType.name}.</p>
                  </div>
                )
              )}
              ...
              {!hasMore && pokemonList.length > 0 && (
                <div className="text-center py-6 text-gray-500 text-sm">
                  Fim da lista de Pokémon do tipo {selectedType.name}.
                </div>
              )}
            </>
          )}
  ...
  // Se nenhum tipo foi selecionado, mostrar a lista de tipos
  return (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {pokemonTypesData.map((type) => (
            <button
              ...
              className={`p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 text-white text-left flex flex-col justify-between h-40 ${type.color}`}
            >
              <div>
                <h2 className="text-2xl font-bold mb-2 capitalize">{type.name}</h2>
